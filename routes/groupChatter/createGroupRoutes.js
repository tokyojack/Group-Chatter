...
module.exports = function(pool) {
    ...
    router.post("/", middleMan.isLoggedIn, function(req, res) {
        pool.getConnection(function(err, connection) {
            ...
            connection.query(createGroup, [req.body.title, req.user.id,  req.user.id], function(err, row) {
                connection.release();

                if (flashUtils.isDatabaseError(req, res, redirectLocation, err))
                    return;

                flashUtils.successMessage(req, res, '/group/'+row[0].insertId, 'Successfully created the group!');
            });
        });
